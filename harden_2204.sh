...
######################################## CIS FUNCTIONS ####################################### 
:'
NO=?;   W=?; S=?; E=; SC=;  BD='SOME TEXT TO DEFINE WHAT IS HAPPENING'
lev && (actions to take) 
'
...
lev && (
  ...
  if lsmod | grep "$l_mname" > /dev/null 2>&1; then 
    echo -e " - unloading module \"$l_mname\"" modprobe -r "$l_mname" 
  fi 
...
NO=1.1.1.2;   W=2; S=2; E=; SC=;  BD='Ensure mounting of squashfs filesystems is disabled'
lev && (
...
NO=1.1.1.3;   W=2; S=2; E=; SC=;  BD='Ensure mounting of jffs2 filesystems is disabled'
lev && (
...
NO=1.1.2.1;   W=1; S=1; E=; SC=;  BD='Ensure /tmp is a separate partition'
lev && (
